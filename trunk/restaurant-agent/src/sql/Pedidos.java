...
public class Pedidos 
{
	public static Receta[] obtenerPlatillos(int cuantos, int aPartirDeCuantos)
	{
		...
		try
		{
			...
			ResultSet rs = bd.realizarQuery("select * from pedidos order by tiempoPedido desc");						
	...
	{		
		BaseDeDatos bd = new BaseDeDatos();				
		bd.ejectuarDML(" insert into pedidos (receta, tiempoPedido) values (" + NoSQLInjection.comillas(rec.clave) + ", now())");
	}
